...
def filter_rules(rules_dict, min_conf, min_body_supp, rule_lengths):
    ...
    for k in rules_dict:
        ...
        if len(new_rules_dict[k]) <= 40:
            new_rules_dict[k] = rules_dict[k][:40]
        # if len(new_rules_dict[k]) >200:
        #     new_rules_dict[k] = new_rules_dict[k][:200]
        # print('before:', len(rules_dict[k]), '  ,after:', len(new_rules_dict[k]))
...
def match_body_relations_reverse(rule, edges, test_query_obj):
    """
    Find edges that could constitute walks (starting from the test query subject)
    that match the rule.
    First, find edges whose subject match the query subject and the relation matches
    ...
    rels = rule["body_rels"][::-1]
    # Match query subject and first body relation
    try:
        rel_edges = edges[rels[0]]
        mask = rel_edges[:, 2] == test_query_obj
        new_edges = rel_edges[mask]
        walk_edges = [
            np.hstack((new_edges[:, 0:1], new_edges[:, 2:4]))
        ]  # [sub, obj, ts]
        cur_targets = np.array(list(set(walk_edges[0][:, 0])))

        for i in range(1, len(rels)):
            # Match current targets and next body relation
            try:
                rel_edges = edges[rels[i]]
                mask = np.any(rel_edges[:, 2] == cur_targets[:, None], axis=0)
                new_edges = rel_edges[mask]
                walk_edges.append(
                    np.hstack((new_edges[:, 0:1], new_edges[:, 2:4]))
                )  # [sub, obj, ts]
                cur_targets = np.array(list(set(walk_edges[i][:, 0])))
            except KeyError:
                walk_edges.append([])
                break
    except KeyError:
...
def get_walks(rule, walk_edges):
    ...
    for i in range(len(walk_edges)):
        ...
        if i==0:
            ...
            rule_walks = pd.merge(rule_walks, df, on=["entity_" + str(i)])
...
def get_candidates(
    ...
    # if 'timestamp_1' in rule_walks.columns and 'timestamp_2' not in rule_walks.columns:
    #     pdb.set_trace()

    for cand in cands:
        cands_walks = rule_walks[rule_walks[max_entity] == cand]
        for s in dicts_idx:
            score = score_func(rule, cands_walks, test_query_ts, *args[s]).astype(np.float32)
            try:
...
def get_grounding_candidates(rule, rule_walks, test_query_ts, cands_dict, rule_id, rule_num):
    """
    Get from the walks that follow the rule the answer candidates.
    Add the confidence of the rule that leads to these candidates.
    ...
    rule_len = len(rule["body_rels"])
    max_entity = "entity_" + str(rule_len)
    cands = set(rule_walks[max_entity])

    for cand in cands:
        all_cands_walks = rule_walks[rule_walks[max_entity] == cand]
        short_cands_walks = all_cands_walks[all_cands_walks["timestamp_0"]>test_query_ts-20]
        latest = test_query_ts - max(all_cands_walks["timestamp_0"])
        lf = len(all_cands_walks)
        sf = len(short_cands_walks)
        if cand not in cands_dict:
            cands_dict[cand] = {}
            cands_dict[cand]['latest'] = np.zeros(rule_num+1, dtype=np.int16)
            cands_dict[cand]['latest'][-1] = 1
            cands_dict[cand]['lf'] = np.zeros(rule_num + 1, dtype=np.int16)
            cands_dict[cand]['lf'][-1] = 1
            cands_dict[cand]['sf'] = np.zeros(rule_num + 1, dtype=np.int16)
            cands_dict[cand]['sf'][-1] = 1
        if rule_len == 1:
            cands_dict[cand]['latest'][rule_id] = latest
            cands_dict[cand]['lf'][rule_id] = lf
            cands_dict[cand]['sf'][rule_id] = sf
        else:
            cands_dict[cand]['latest'][rule_id] = latest
            cands_dict[cand]['lf'][rule_id] = min(lf,1)
            cands_dict[cand]['sf'][rule_id] = min(sf,1)
