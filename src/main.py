...
from models import LCWT, LCFT, LTV, Noisy_OR, TempValid, FETA

parser = argparse.ArgumentParser()
parser.add_argument("--dataset", "-d", default="ICEWS14", type=str)
parser.add_argument("--model", "-m", default="FETA", type=str)  # LCWT: without time, LCFT: static time, LTV: learning time, Noisy_OR: Noisy_OR score
...
parser.add_argument("--rate", "-r", default=-1, type=int)
parser.add_argument("--neg_num", "-n", default=100, type=int)
parser.add_argument("--max_epoch", "-e", default=1000, type=int)
parser.add_argument("--batch_size", default=1024, type=int)
parser.add_argument("--valid_epoch", "-v", default=5, type=int)
...
parser.add_argument("--fluctuation", default="para", type=str) # 1) para: FETA; 2) dense 3) fixed
...
def conf_learning(args, rel, train_data, valid_data, conf_tensor=None):
    if args.rate>0:
        start_num = 2**args.rate
        train_num = start_num
    else:
        start_num = 0
        train_num = len(train_data[0])
    rule_dim = train_data[0][0].shape[1]
    train_dataset = Dataset((train_data[0][-start_num:],train_data[1][-start_num:],train_data[2][-start_num:]), args, split='train')
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)

    valid_num = 0
    if valid_data!= None and len(valid_data) >0:
        valid_num = len(valid_data[0])
    ...
    with tqdm(total=args.max_epoch) as _tqdm:
        for epoch in range(args.max_epoch):
            ...
            epoch_loss = 0
            epoch_num = 0
            for i_batch, (latest_data, lf_data, sf_data, neg_masks) in enumerate(train_loader):
                if args.cuda:
            ...
            if decay_count > 2*args.valid_epoch and break_count>5:
            ...
            if break_count>20:
                ...
                break
    # logging.info('min_loss:{0}, valid_num:{1}, , valid_mrr:{2}\n'.format(round(min_valid_loss, 5), valid_num, round(max_mrr, 5)))
    # print('rel_id: {0}, rule_num:{1}, train_num:{2}'.format(rel, rule_dim - 2, train_num))
    # print('min_loss:{0}, valid_num:{1}, , valid_mrr:{2}\n'.format(round(min_valid_loss, 5), valid_num, round(max_mrr, 5)))
    logging.info('rel_id: {0}, rule_num:{1}, train_num:{2}'.format(rel, rule_dim - 2, train_num))
    logging.info('min_loss:{0}, valid_num:{1}, , valid_mrr:{2}\n'.format(round(min_valid_loss, 5), valid_num, round(max_mrr, 5)))
    # test_mrr =0
    # if test_loader != None:
    #     model.load_state_dict(valid_model_dict)
    #     ranks, test_loss = evaluate(args, test_loader, model)
    #     test_mrr = torch.sum(1 / ranks).item()
...
def evaluation(split='test'):
    eval_array = np.zeros(4)
    cover_count = 0
    all_count = 0
    for rel in rels:
        test_data = load_data(args, rel, split=split)
        if split=='test':
            test_data_num = quadruple_data.test_idx[quadruple_data.test_idx[:, 1] == rel].shape[0]
        else:
            test_data_num = quadruple_data.valid_idx[quadruple_data.valid_idx[:, 1] == rel].shape[0]
        all_count += test_data_num
        model_path = model_dir + 'rel_{}.pth'.format(rel)
        if test_data != None and test_data_num > 0 and os.path.isfile(model_path):
            test_num = len(test_data[0])
            cover_count += test_num
            test_dataset = Dataset(test_data, args, split=split)
