...
output_dir = dir_path + split +'_cross' + '/'
...
def create_data(i, num_queries, neg_num=30, split='test'):
    ...
    for j_id, j in enumerate(process_queries_idx):
        ...
        cands_dict = {}
        pos_latest = np.zeros(rule_num+1, dtype=np.int16)
        pos_lf = np.zeros(rule_num+1, dtype=np.int16)
        pos_sf = np.zeros(rule_num+1, dtype=np.int16)
        # t1 = time.time()
        ...
        if obj in cands_dict:
            pos_latest = cands_dict[obj]['latest']
            pos_lf = cands_dict[obj]['lf']
            pos_sf = cands_dict[obj]['sf']

        # filter other positive cand
        cands_dict = {cand: cand_dict for cand, cand_dict in cands_dict.items() if cand not in pos_cands}
        ...
        if split != 'test':
            cands_score = {}
            # rule-adversarial
            for cand, cand_dict in cands_dict.items():
                # Score by TLogic
                cand_array = cand_dict['latest']
            ...
            neg_latest_arr = np.array([cands_dict[neg_cand]['latest'] for neg_cand in neg_cand_list], dtype=np.int16)
            neg_lf_arr = np.array([cands_dict[neg_cand]['lf'] for neg_cand in neg_cand_list], dtype=np.int16)
            neg_sf_arr = np.array([cands_dict[neg_cand]['sf'] for neg_cand in neg_cand_list], dtype=np.int16)
            actual_cand_num = len(neg_cand_list)
        else:
            neg_latest_arr = np.array([cands_dict[neg_cand]['latest'] for neg_cand in cands_dict], dtype=np.int16)
            neg_lf_arr = np.array([cands_dict[neg_cand]['lf'] for neg_cand in cands_dict], dtype=np.int16)
            neg_sf_arr = np.array([cands_dict[neg_cand]['sf'] for neg_cand in cands_dict], dtype=np.int16)
            actual_cand_num = len(cands_dict)
        if actual_cand_num < neg_num:
            pad_arr = np.zeros((neg_num - actual_cand_num, rule_num + 1), dtype=np.int16)
            neg_latest_arr = neg_latest_arr.reshape(-1, rule_num + 1)
            neg_lf_arr = neg_lf_arr.reshape(-1, rule_num + 1)
            neg_sf_arr = neg_sf_arr.reshape(-1, rule_num + 1)
            neg_latest_arr = np.concatenate((neg_latest_arr, pad_arr), axis=0)
            neg_lf_arr = np.concatenate((neg_lf_arr, pad_arr), axis=0)
            neg_sf_arr = np.concatenate((neg_sf_arr, pad_arr), axis=0)

        pos_latest = pos_latest.reshape(1, -1)
        pos_lf = pos_lf.reshape(1, -1)
        pos_sf = pos_sf.reshape(1, -1)
        latest_output = np.concatenate((pos_latest, neg_latest_arr), axis=0)
        lf_output = np.concatenate((pos_lf, neg_lf_arr), axis=0)
        sf_output = np.concatenate((pos_sf, neg_sf_arr), axis=0)
        # save memory by sparse matrix
        row, col = np.nonzero(latest_output)
        values = latest_output[row, col]
        query_csr = csr_matrix((values, (row, col)), shape=latest_output.shape)
        output_dict[j]['latest'] = query_csr
        row, col = np.nonzero(lf_output)
        values = lf_output[row, col]
        query_csr = csr_matrix((values, (row, col)), shape=lf_output.shape)
        output_dict[j]['lf'] = query_csr
        row, col = np.nonzero(sf_output)
        values = sf_output[row, col]
        query_csr = csr_matrix((values, (row, col)), shape=sf_output.shape)
        output_dict[j]['sf'] = query_csr
        ...
        if rel in rules_dict:
            for i in range(num_processes):
                for query_id in output[i].keys():
                    query_csr = output[i][query_id]['latest']
                    latest_list.append(query_csr)
                    query_csr = output[i][query_id]['lf']
                    lf_list.append(query_csr)
                    query_csr = output[i][query_id]['sf']
                    sf_list.append(query_csr)

            count += len(latest_list)
            filename = "{0}_{1}_latest.pkl".format(split, rel)
            pickle.dump(latest_list, open(output_dir + filename, "wb"))
            filename = "{0}_{1}_lf.pkl".format(split, rel)
            pickle.dump(lf_list, open(output_dir + filename, "wb"))
            filename = "{0}_{1}_sf.pkl".format(split, rel)
            pickle.dump(sf_list, open(output_dir + filename, "wb"))
